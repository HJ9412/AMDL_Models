...
class Encoder(nn.Module):
    def __init__(self, data_channels, channels_A, channels_B, channels_C, channels_D, latent_dim):
        ...
        self.fc_DLmean  =nn.Linear(channels_D*16, latent_dim)
        self.fc_DLvar   =nn.Linear(channels_D*16, latent_dim)
...
class Decoder(nn.Module):
    def __init__(self, latent_dim, channels_D, channels_C, channels_B, channels_A, data_channels):
        ...
        # Layers
        self.fc_LD =nn.Linear(latent_dim, channels_D*16)
...
class Evaluator(nn.Module):
    def __init__(self, data_dim, hidden_dim1, hidden_dim2, value_dim):
        ...
        # Layers
        self.fc_x1hA = nn.Linear(data_dim, hidden_dim1)
        self.fc_x2hB = nn.Linear(data_dim, hidden_dim1)
        self.fc_hAA = nn.Linear(hidden_dim1, hidden_dim2)
        self.fc_hBA = nn.Linear(hidden_dim1, hidden_dim2)
        self.fc_hAB = nn.Linear(hidden_dim1, hidden_dim2)
        self.fc_hBB = nn.Linear(hidden_dim1, hidden_dim2)
        self.fc_eval = nn.Linear(hidden_dim2*2, value_dim)

    def forward(self, x1, x2):
        x1 = torch.flatten(x1, start_dim = 1)
        x2 = torch.flatten(x2, start_dim = 1)
        hA = F.relu(self.fc_x1hA(x1))
        hB = F.relu(self.fc_x2hB(x2))
        hAA = F.relu(self.fc_hAA(hA))
        hBA = F.relu(self.fc_hBA(hB))
        hAB = F.relu(self.fc_hAA(hA))
        hBB = F.relu(self.fc_hBB(hB))
        h2 = torch.concat((hAA+hBA, hAB+hBB), dim=1)
        values = F.relu(self.fc_eval(h2))
        return values
...
if __name__ == '__main__':
    ...
    transform = transforms.Compose([
        ...
        # Datasets
    data_train = EMNIST(dataset_folder, split='byclass', train=True, transform=transform)
    data_test = EMNIST(dataset_folder, split='byclass', train=False, transform=transform)
    ...
    for epoch in range(VAE_num_epochs):
        ...
        if sample_train_data and epoch % sampling_epoch == sampling_epoch - 1:
            displayer(sampling, x.shape[0], data_x, data_y, data_is_rgb)
    optimizer_VAE.zero_grad()
    ...
    # Training Evaluator
    optimizer_Eval = optim.Adam(evaluator.parameters(), lr=0.00002)
    ...
    for epoch in range(EVAE_num_epochs):
        ...
        for batch_idx, (data, data_label) in enumerate(data_loader):
            ...
            kl_loss = -0.5 * torch.sum(1 + log_var - mean.pow(2) - log_var.exp())
            eval_loss = torch.sum(torch.exp(-values))
            total_loss = recon_loss + kl_loss + eval_loss
            ...
            if (batch_idx+1) * batch_size % log_interval == 0:
                print(f"[{(batch_idx+1) * batch_size}/{len(data_loader.dataset)}] recon_loss: {recon_loss.item() / len(data):.2f}  kl_loss: {kl_loss.item() / len(data):.2f}  eval_loss: {eval_loss.item() / len(data):.2f}  Total Loss: {total_loss / len(data):.2f}")
    ...
    # Testing
    data_loader = DataLoader(data_test, batch_size = 100)
    ...
    if data_is_rgb:
        for data, data_label in data_loader:
            ...
            values = evaluator(x, image_origin.reshape(1,-1))
        ...
        for data, data_label in data_loader:
            ...
            for i in range(1, test_sampling):
                ...
                values = evaluator(image.reshape(1,-1), recon_x.reshape(1,-1))
