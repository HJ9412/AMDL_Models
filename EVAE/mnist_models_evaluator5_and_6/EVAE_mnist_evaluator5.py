...
class Evaluator(nn.Module):    # Non-biased Feature Extractor
    def __init__(self, data_dim, hidden_dim1, hidden_dim2, value_dim):
        ...
        self.fc_eval = nn.Linear(2*hidden_dim2, value_dim)
...
if __name__ == '__main__':
    ...
    for epoch in range(EVAE_num_epochs):
        ...
        for batch_idx, (data, data_label) in enumerate(data_loader):  # Assuming x is a batch from your data
            ...
            if (batch_idx+1) * batch_size % log_interval == 0:
                print(f"[{(batch_idx+1) * batch_size}/{len(data_loader.dataset)}] recon_loss: {recon_loss.item() / len(data):.1f}  kl_loss: {kl_loss.item() / len(data):.2f}  VAE_loss: {VAE_loss.item()/len(data):.1f}  eval_loss: {eval_loss.item() / len(data):.2f}")
